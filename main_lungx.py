...
hhh = 0
useCheckpoint = True
# my_resume = '/home/zhangconghao/VUnet_1/data/fold_1ckpt.t7'
my_resume = '/home/zhangconghao/VUnet_1/results/ablation/FL_Adan_fold_5ckpt.t7'
...
def main():
    ...
    for fold in range(start_fold, end_fold):
        ...
        dataset = data_lungx.LunaClassifier(
            fold,
            phase='val')
...
def test_EBAM(data_loader, net):
    ...
    feats = targs = predicts =  np.array([])
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(data_loader):
            inputs = Variable(inputs.cuda(non_blocking = True)).reshape(targets.size(0),1,32,32,32)
    ...
    # TSNE
            ...
            predicts = np.append(predicts, predicted.data.cpu().numpy())
            correct += predicted.cpu().eq(targets.data).sum()
            # # error_idx.append(idxs[predicted.eq(targets.data).cpu()])
        metrics.roc_auc_score(targs, predicts)
        auc = metrics.auc(fpr, tpr)
        print(correct/len(targs))
        print(auc)
        print('')
