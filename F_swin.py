...
class SwinUNETR(nn.Module):
    ...
    def forward(self, x_in):
        ...
        # dec3 = self.decoder5(dec4, hidden_states_out[2])
        ...
        dec0 = self.transp_conv1(dec0)
        out = self.decoder1(dec0, enc0)
        out = self.out(out)
        out = self.endBlock(out)
        out1 = out.view(out.size(0), -1)
        # mlp = self.con_mlp(out)
        feats = self.classifier_mlp[:2](out1)
        out = self.classifier_mlp(out1)
