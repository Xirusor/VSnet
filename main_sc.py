...
useVisdom = True
global mySave_dir
mySave_dir = 'FL_BS8_gamma0.5_Adan_random4'
...
def main():
    ...
    for fold in range(start_fold,end_fold):
        ...
        loss_focal, loss_sc = FocalLoss(gamma=0.5), SupConLoss()
        loss_focal, loss_sc = loss_focal.cuda(), loss_sc.cuda()
        ...
        if args.test:
            test_EBAM(val_loader, net1, net2, loss2)
            sys.exit()
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            train(train_loader, swin_net, loss_sc, loss_focal, epoch, optimizer_swin, get_lr, logging, viz, step_list, train_loss, two_acc)
            Evaluate('val', val_loader, swin_net, loss_focal, epoch, mySave_dir, str(fold), logging, viz, step_list, train_loss, two_acc, tpr_tnr)
        print('finished!')


def train(data_loader, swin_net, loss_sc, loss_focal, epoch, optimizer_swin, get_lr, logging, viz, step_list, train_loss_viz, two_acc):
    ...
    for i, (data1, data2, targets) in enumerate(data_loader):
        images = torch.cat([data1, data2], dim=0)
        bsz = targets.shape[0]
        if torch.cuda.is_available():
            images = images.cuda(non_blocking=True)
            targets = targets.cuda(non_blocking=True).to(torch.long)
        
        features, _ = swin_net(images)
        f1, f2 = torch.split(features, [bsz, bsz], dim=0)
        features = torch.cat([f1.unsqueeze(1), f2.unsqueeze(1)], dim=1)
        loss_mlp = loss_sc(features, targets)
        optimizer_swin.zero_grad()
        loss_mlp.backward()
        optimizer_swin.step()
        train_loss_sc += loss_mlp.data.item()
