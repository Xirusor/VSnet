...
import sys
from networks.UXNet_3D.network_backbone import UXNET, CenterLoss
import numpy as np
import albumentations as al
from swin_unet_mlp import FocalLoss
sys.path.append('/')
...
def main():
    ...
    for fold in range(start_fold, end_fold):
        ...
        for epoch in range(0, args.epochs + 1):
            train(train_loader, swin_unet, loss_focal, centerloss, epoch, optimizer_swin, logging, get_lr)
            Evaluate('val', val_loader, swin_unet, loss_focal, epoch, mySave_dir, str(fold), logging)
            # test_EBAM(val_loader, swin_unet)


def train(data_loader, swin_net, loss_focal, centerloss, epoch, optimizer_swin, logging, get_lr):
    ...
    for i, (img, _, tars) in enumerate(data_loader):
        ...
        loss_center = centerloss(feats, tars)
        if loss_center>0:
            loss = loss_fc + 0.03 * loss_center
            train_loss_center += loss_center.data.item()*0.003
        else:
            loss = loss_fc
        loss = loss_fc
    ...
    #     lr,
    #     spend_time))
    logging.info('\n    Epochs:{:>4d} \n       Acc:{:>6.2f}, loss1:{:>6.5f},loss1:{:>6.5f}, lr:{:.2e}, time:{:>5.2f}'.format(
        epoch,
        acc,
        train_loss_focal/total,
        train_loss_center/total,
        lr,
        spend_time))
